    ...
    for (let i = 0; i < params.testCases.length; i++) {
      ...
      if (gradingResult.exitCode !== 0) {
        return {
          ...
          failedTestIndex: i,
          stderr: 'Sandbox functional runner failed to execute.',
          message: `Test #${i + 1} crashed inside the sandbox.`,
        }
      }
      ...
      if (executionResult.status !== 'accepted' || (executionResult.exitCode ?? 1) !== 0) {
        return {
          ...
          failureKind: 'runtime_error',
          failedTestIndex: i,
          stderr: executionResult.stderr || executionResult.error || 'Sandbox execution failed.',
          message: `Test #${i + 1} crashed inside the sandbox.`,
        }
      }
